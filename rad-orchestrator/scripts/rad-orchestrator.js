...
${RAD_DOCX_ULAZ ? `- POSTOJEĆI RAD (mod 4/6): ${PROJECT_ROOT}/rad.docx je IZVOR ISTINE, nema plan.json ni .katedra/poglavlja/. NIKAD ne pokreći build_docx.py ni rukopis.py. Popravci idu ISKLJUČIVO na kopiju: snapshot → fix_rules.py --out / apply_safe_fixes.py / sigurni_popravci_hr.py → verify_rewrite.py prije poslije → revizije.py redline. Sadržajni nalazi (struktura, argument, izvori) su treba_autora, ne mijenjaju se — OSIM kad su u ODLUKAMA AUTORA (npr. mentorove primjedbe koje korisnik traži provesti): tada se provode na kopiji, a svaka namjerno promijenjena brojka ili citatni ključ ide u verify_rewrite.py --namjerno TOKEN=RAZLOG. Nakon svake izmjene koja pomiče prijelom: python3 ${S}/revizije.py toc ./rad_v2.docx ./rad_v2.docx.` : ''}
